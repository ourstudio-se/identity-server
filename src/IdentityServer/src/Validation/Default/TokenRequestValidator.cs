...
using Ourstudio.IdentityServer.Events;
using Ourstudio.IdentityServer.Logging.Models;
...
namespace Ourstudio.IdentityServer.Validation
{
    internal class TokenRequestValidator : ITokenRequestValidator
    {
            ...
            NameValueCollection parameters)
        {
            ...
            _logger.LogTrace("Calling into custom request validator: {type}",
                _customRequestValidator.GetType().FullName);
            ...
            NameValueCollection parameters)
        {
            ...
            _logger.LogDebug("{clientId} credentials token request validation success",
            ...
            NameValueCollection parameters)
        {
            ...
            if (resourceOwnerContext.Result.IsError)
            {
                ...
                if (resourceOwnerContext.Result.Error == OidcConstants.TokenErrors.UnsupportedGrantType)
                {
                    ...
                    await RaiseFailedResourceOwnerAuthenticationEventAsync(userName,
                        "password grant type not supported", resourceOwnerContext.Request.Client.ClientId);
            ...
            {
                LogError("User has been disabled",
                    new { subjectId = resourceOwnerContext.Result.Subject.GetSubjectId() });
        ...
        private async Task<TokenRequestValidationResult> ValidateRefreshTokenRequestAsync(
            NameValueCollection parameters)
        {
            ...
            var result =
                await _refreshTokenService.ValidateRefreshTokenAsync(refreshTokenHandle, _validatedRequest.Client);
            ...
            NameValueCollection parameters)
        {
                    ...
                    .Contains(_validatedRequest.GrantType, StringComparer.Ordinal))
            {
                LogError("No validator is registered for the grant type",
                    new { grantType = _validatedRequest.GrantType });
                return Invalid(OidcConstants.TokenErrors.UnsupportedGrantType);
            }
            ...
            AuthorizationCode authZcode)
        {
            ...
            {
                LogError("Unsupported code challenge method",
                    new { codeChallengeMethod = authZcode.CodeChallengeMethod });
                return Invalid(OidcConstants.TokenErrors.InvalidGrant);
            }
            ...
            string clientId)
        {
            return _events.RaiseAsync(
                new UserLoginSuccessEvent(userName, subjectId, null, interactive: false, clientId));
        }

        private Task RaiseFailedResourceOwnerAuthenticationEventAsync(string userName, string error, string clientId)
        {
            return _events.RaiseAsync(
                new UserLoginFailureEvent(userName, error, interactive: false, clientId: clientId));
        }
    }
}