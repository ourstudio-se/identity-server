...
using Ourstudio.IdentityServer.Services.Default;
...
namespace Ourstudio.IdentityServer.UnitTests.ResponseHandling
{
    public class DeviceAuthorizationResponseGeneratorTests
    {
        ...
        private readonly List<ApiResource> apiResources = new List<ApiResource>
            { new ApiResource("resource") { Scopes = { "api1" } } };
        ...
        private readonly IDeviceFlowCodeService deviceFlowCodeService =
            new DefaultDeviceFlowCodeService(new InMemoryDeviceFlowStore(), new StubHandleGenerationService());
        ...
        public DeviceAuthorizationResponseGeneratorTests()
        {
            ...
            generator = new DeviceAuthorizationResponseGenerator(
                options,
                new DefaultUserCodeService(new IUserCodeGenerator[]
                    { new NumericUserCodeGenerator(), fakeUserCodeGenerator }),
        ...
        public async Task ProcessAsync_when_user_code_collision_expect_retry()
        {
            ...
            await deviceFlowCodeService.StoreDeviceAuthorizationAsync(FakeUserCodeGenerator.TestCollisionUserCode,
                new DeviceCode());
