...
using Ourstudio.IdentityServer.IntegrationTests.Common;
using Xunit;
using JsonClaimValueTypes = Microsoft.IdentityModel.JsonWebTokens.JsonClaimValueTypes;
using JsonSerializer = System.Text.Json.JsonSerializer;
using JsonWebKey = Microsoft.IdentityModel.Tokens.JsonWebKey;

namespace Ourstudio.IdentityServer.IntegrationTests.Endpoints.Authorize
{
    public class JwtRequestAuthorizeTests
    {
                    ...
                    ClientSecrets =
                    {
                        ...
                        new Secret
                        {
                            ...
                            Value = JsonConvert.SerializeObject(
                                JsonWebKeyConverter.ConvertFromX509SecurityKey(new X509SecurityKey(TestCert.Load())))
                        }
                    },
        ...
        public async Task authorize_should_accept_valid_JWT_request_object_parameters_using_symmetric_jwk()
        {
            var requestJwt = CreateRequestJwt(
                issuer: _client.ClientId,
                audience: IdentityServerPipeline.BaseUrl,
                credential: new SigningCredentials(new JsonWebKey(_symmetricJwk), "HS256"),
                claims: new[]
                {
                    new Claim("client_id", _client.ClientId),
                    new Claim("response_type", "id_token"),
        ...
        public async Task authorize_should_accept_complex_objects_in_request_object()
        {
            ...
            var someObjJson = JsonSerializer.Serialize(someObj);// JsonConvert.SerializeObject(someObj);
            var someArr = new[] { "a", "b", "c" };
            var someArrJson = JsonSerializer.Serialize(someArr);//JsonConvert.SerializeObject(someArr));
            ...
            var someObj2 =
                JsonConvert.DeserializeObject(_mockPipeline.LoginRequest.Parameters["someObj"], someObj.GetType());
            ...
            someObj2 = JsonConvert.DeserializeObject(_mockPipeline.LoginRequest.RequestObjectValues["someObj"],
                someObj.GetType());
            someObj.Should().BeEquivalentTo(someObj2);
        ...
        public async Task request_uri_response_returns_404_should_fail()
        {
            ...
            _mockPipeline.JwtRequestMessageHandler.Response = new HttpResponseMessage(HttpStatusCode.NotFound);
