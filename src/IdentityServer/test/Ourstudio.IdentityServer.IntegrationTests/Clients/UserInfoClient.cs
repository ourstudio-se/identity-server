...
using Ourstudio.IdentityServer.IntegrationTests.Clients.Setup;
using Xunit;

namespace Ourstudio.IdentityServer.IntegrationTests.Clients
{
    public class UserInfoEndpointClient
    {
        ...
        public async Task Complex_json_should_be_correct()
        {
            ...
            var scopes = payload["scope"].As<JArray>().Values<string>().ToArray();
            ...
            var roles = payload["role"].As<JArray>().Values<string>().ToArray();
